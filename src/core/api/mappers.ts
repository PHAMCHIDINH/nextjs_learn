...
const REPORT_STATUSES: Array<Report["status"]> = [
  "pending",
  "reviewed",
  "resolved",
];
const MESSAGE_TYPES: Array<Message["type"]> = ["text", "image"];
const MODERATION_RISK_LEVELS: Array<ListingModeration["riskLevel"]> = [
  "low",
  "medium",
  "high",
  "error",
];
const MODERATION_RECOMMENDED_ACTIONS: Array<
  NonNullable<ListingModeration["recommendedAction"]>
> = ["approve", "manual_review"];
const MODERATION_JOB_STATUSES: ModerationJobStatus[] = [
  "pending",
  "running",
  "completed",
  "failed",
...
const normalizeDepartmentOptional = (
  value: unknown,
...
const normalizeModerationRiskLevel = (
  value: unknown,
): ListingModeration["riskLevel"] =>
  normalizeEnum(value, MODERATION_RISK_LEVELS, "error");

const normalizeModerationAction = (
  value: unknown,
): ListingModeration["recommendedAction"] => {
  if (typeof value !== "string") {
    return null;
  }

  return (MODERATION_RECOMMENDED_ACTIONS as readonly string[]).includes(value)
    ? (value as NonNullable<ListingModeration["recommendedAction"]>)
    : null;
};

const normalizeModerationJobStatus = (
  value: unknown,
): ModerationJobStatus | undefined => {
  if (typeof value !== "string") {
    return undefined;
  }

  return (MODERATION_JOB_STATUSES as readonly string[]).includes(value)
    ? (value as ModerationJobStatus)
    : undefined;
};
...
export const mapProduct = (value: unknown): Product => {
  ...
  const latestModeration = isRecord(source.latestModeration)
    ? {
        riskLevel: normalizeModerationRiskLevel(
          source.latestModeration.riskLevel,
        ),
        confidence:
          source.latestModeration.confidence === null ||
          source.latestModeration.confidence === undefined
            ? null
            : asNumber(source.latestModeration.confidence),
        recommendedAction: normalizeModerationAction(
          source.latestModeration.recommendedAction,
        ),
        summary:
          source.latestModeration.summary === null ||
          source.latestModeration.summary === undefined
            ? null
            : asString(source.latestModeration.summary),
        violations: Array.isArray(source.latestModeration.violations)
          ? source.latestModeration.violations.filter(
              (item): item is string => typeof item === "string",
            )
          : [],
        createdAt: toDate(source.latestModeration.createdAt),
      }
    : undefined;
  ...
  return {
    ...
    originalPrice:
      source.originalPrice === undefined || source.originalPrice === null
        ? undefined
        : asNumber(source.originalPrice),
    images,
    ...
    department: source.department
      ? normalizeDepartment(source.department)
      : seller.department,
    createdAt: toDate(source.createdAt),
    ...
    isSaved:
      source.isSaved === undefined ? undefined : asBoolean(source.isSaved),
    approvalStatus:
      ...
      source.approvalStatus === "rejected"
        ...
        : undefined,
    latestModeration,
    moderationJobStatus: normalizeModerationJobStatus(
      source.moderationJobStatus,
    ),
  };
};
...
export const mapConversation = (value: unknown): Conversation => {
  ...
  const participants = Array.isArray(source.participants)
    ? source.participants.map(mapUser)
    : [];
  return {
    id: asString(source.id),
    participants,
    lastMessage: source.lastMessage
      ? mapMessage(source.lastMessage)
      : undefined,
...
export const mapPaginatedReports = (
  value: unknown,
